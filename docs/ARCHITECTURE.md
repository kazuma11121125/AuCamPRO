...
上記の修正によりVideo/Audio双方が妥当なフレーム数・妥当な終端ズレでファイルに書き込まれることは確認できたが、§4.3の最重要要件である**±20ms A/V同期の実測はまだ行っていない**。拍手やフラッシュ等の物理的な同期基準点を録画クリップに含める作業は、カメラの前で実際に手を叩く/発光させるという物理操作が必要であり、リモート開発環境からは実行できない——ユーザーに実機を使った同期検証を依頼する必要がある。
